...
Moments is the first featured project and links to `moments.html`. Its cover uses two photo cards and a pink-red heart. The detail page includes a presentation mockup with sample content and an accessible native-dialog lightbox. DTRCam and Server Infrastructure each have a matching project page: `dtrcam.html` uses an optimized WebP mockup with sample personal details, while `server-infra.html` uses a code-native infrastructure diagram and intentionally omits internal addresses and credentials. Older screenshots remain in `static/` but are not loaded by the page.
